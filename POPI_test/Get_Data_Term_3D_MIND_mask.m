...
function unary_tot=Get_Data_Term_3D_MIND_mask(Mov_mind,Fix_mind,seg_mask_mov,seg_mask_fix,griddim,O_trans_X,O_trans_Y,O_trans_Z,labels,level,previous,quant)
...
seg_mask_fix_large = uncrop_data_xp_3d(seg_mask_fix, crop_v, size_orig,0);
seg_mask_mov_large = uncrop_data_xp_3d(seg_mask_mov, crop_v, size_orig,0);
for R_index=1:R

    volmovl= uncrop_data_xp_3d(Mov_mind(:,:,:,R_index), crop_v, size_orig,weight);
    volmovl = volmovl.*seg_mask_mov_large;

    volfixl= uncrop_data_xp_3d(Fix_mind(:,:,:,R_index), crop_v, size_orig,weight);
    volfixl = volfixl.*seg_mask_fix_large;
